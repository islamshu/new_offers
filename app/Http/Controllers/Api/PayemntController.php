...
class PayemntController extends BaseController
{
    ...
    public function myfatoorah2(Request $request)
    {
        // dd($request->package_id);
        $code = Subscription::find($request->package_id);
        if (!$code) {
        ...
        if (isset($json->IsSuccess) && $json->IsSuccess == true) {
            ...
            $payment = new Payment();
            $payment->order_id = Carbon::now()->timestamp;
            $payment->invoice_id = $json->Data->InvoiceId;
    ...
    public  function active(Request $request)
    {
        ...
        if (isset($json->IsSuccess) && $json->IsSuccess == true) {
            if($json->Data->InvoiceStatus == 'Paid'){
               ...
               $user->sub_id  = $code->id;
               $user->clinet_id  = auth('client_api')->id();
               $user->promocode = $payment->promocode;
               $user->save();
    ...
    {
        $payment = Payment::where('order_id',(string)$request->order_id)->where('invoice_id',(string)$request->invoice_id)->first();
